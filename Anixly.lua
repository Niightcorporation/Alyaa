...
RunService.RenderStepped:Connect(function()
    pcall(function()
        local focusNav = CoreGui.RobloxGui.FocusNavigationCoreScriptsWrapper
...
local function applyTheme(themeName)
    ...
    if UI_Elements.SkinDropBtn then
        UI_Elements.SkinDropBtn.BackgroundColor3 = theme.mid
    end
    
    if UI_Elements.SkinDropGrad then
        UI_Elements.SkinDropGrad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, theme.primary),
            ColorSequenceKeypoint.new(1, theme.dark)
        })
    end
    
    if UI_Elements.SkinDropStroke then
        UI_Elements.SkinDropStroke.Color = theme.mid
    end
    
    if UI_Elements.SkinFrameStroke then
        UI_Elements.SkinFrameStroke.Color = theme.mid
    end
    
    if UI_Elements.SkinInfoText then
        UI_Elements.SkinInfoText.TextColor3 = theme.logText
    end
...
MiniIcon.Image = "rbxassetid://99683372434053"
...
-- Fake Tab
local fakeContainer = createTabContainer()
fakeContainer.ScrollingDirection = Enum.ScrollingDirection.Y
...
local fakeTab = createTabButton("🎭", "FAKE", 2)
local utilTab = createTabButton("🔧", "UTIL", 3)
local tpTab = createTabButton("🎋", "TP", 4)
...
local function createSpeedSettings(parent, order)
    ...
    local totalHeight = (28 + 4 * itemHeight) + 6
    ...
    local sliders = {
        ...
        },
        {
            ...
            cbMax = function(v) rejoinDelay = v end
        },
        {
            icon = "⏎",
            label = "ENTER",
    ...
    for i, s in ipairs(sliders) do
        ...
        iconLabel.Size = UDim2.new(0, 55, 0, 14)
        iconLabel.Position = UDim2.new(0, 8, 0, 4)
        iconLabel.BackgroundTransparency = 1
        iconLabel.Text = s.icon .. " " .. s.label
...
-- Add speed settings
createSpeedSettings(mainContainer, 31)
...
createToggleButton("AUTO TULIS", mainContainer, false, function(state)
    ...
    if autoTypeEnabled then
        task.wait(0.3)
        -- autoType() function will be called from remote handler
    end
end, 1)
...
createToggleButton("AUTO REJOIN", mainContainer, true, function(state)
    autoRejoinEnabled = state
end, 3)

createToggleButton("🧠 MANUSIA [PERCOBAAN]", mainContainer, false, function(state)
...
local function updateCategoryButtons()
    ...
    for i, cat in ipairs(categories) do
        ...
        btn.Text = (isOn and "✓  " or "   ") .. cat .. (isOn and "  [ON]" or "")
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if cat == "SEMUA KATA SULIT" then
                ...
                if categoryToggles["SEMUA KATA SULIT"] then
                    categoryToggles.IF = false
                    categoryToggles.X = false
                    categoryToggles.NG = false
                    categoryToggles.AI = false
                    categoryToggles.CY = false
                    categoryToggles.UI = false
                    categoryToggles.KS = false
                end
            else
...
awalanLabel.Size = UDim2.new(1, -12, 0, 22)
awalanLabel.Position = UDim2.new(0, 8, 0, 4)
...
-- Fake Tab Content
local fakePadding = Instance.new("UIPadding")
fakePadding.PaddingLeft = UDim.new(0, 6)
fakePadding.PaddingRight = UDim.new(0, 6)
fakePadding.PaddingTop = UDim.new(0, 8)
fakePadding.Parent = fakeContainer

local fakeTitle = Instance.new("TextLabel")
fakeTitle.Size = UDim2.new(1, 0, 0, 18)
fakeTitle.Position = UDim2.new(0, 0, 0, 0)
fakeTitle.BackgroundTransparency = 1
fakeTitle.Text = "GANTI NAMA TAMPILAN"
fakeTitle.TextColor3 = Color3.fromRGB(120, 80, 200)
fakeTitle.Font = Enum.Font.GothamBold
fakeTitle.TextSize = 10
fakeTitle.TextXAlignment = Enum.TextXAlignment.Left
fakeTitle.Parent = fakeContainer

local nameBox = Instance.new("TextBox")
nameBox.Size = UDim2.new(1, 0, 0, IsMobile and 44 or 38)
nameBox.Position = UDim2.new(0, 0, 0, 24)
nameBox.BackgroundColor3 = Color3.fromRGB(18, 16, 28)
nameBox.PlaceholderText = "Masukkan Nama Palsu..."
nameBox.Text = ""
nameBox.TextColor3 = Color3.new(1, 1, 1)
nameBox.Font = Enum.Font.GothamBold
nameBox.TextSize = TEXT_SIZE_NORMAL + 1
nameBox.PlaceholderColor3 = Color3.fromRGB(90, 80, 120)
nameBox.Parent = fakeContainer

local nameBoxCorner = Instance.new("UICorner")
nameBoxCorner.CornerRadius = UDim.new(0, 9)
nameBoxCorner.Parent = nameBox

local nameBoxStroke = Instance.new("UIStroke")
nameBoxStroke.Color = Color3.fromRGB(90, 45, 180)
nameBoxStroke.Thickness = 1.5
nameBoxStroke.Transparency = 0.2
...
local applyBtnY = IsMobile and 76 or 68

local applyBtn = Instance.new("TextButton")
applyBtn.Size = UDim2.new(1, 0, 0, IsMobile and 44 or 38)
applyBtn.Position = UDim2.new(0, 0, 0, applyBtnY)
applyBtn.BackgroundColor3 = Color3.fromRGB(60, 130, 240)
applyBtn.Text = "✏️ TERAPKAN NAMA"
applyBtn.TextColor3 = Color3.new(1, 1, 1)
applyBtn.Font = Enum.Font.GothamBold
applyBtn.TextSize = TEXT_SIZE_NORMAL + 1
applyBtn.Parent = fakeContainer
...
local infoY = applyBtnY + (IsMobile and 52 or 46)

local infoLabel = Instance.new("TextLabel")
infoLabel.Size = UDim2.new(1, 0, 0, 50)
infoLabel.Position = UDim2.new(0, 0, 0, infoY)
infoLabel.BackgroundTransparency = 1
infoLabel.Text = "⚠️  Nama palsu hanya terlihat visual.\nJika tidak berhasil, coba rejoin server."
infoLabel.TextColor3 = Color3.fromRGB(120, 115, 150)
infoLabel.Font = Enum.Font.Gotham
infoLabel.TextSize = 10
infoLabel.TextWrapped = true
infoLabel.TextXAlignment = Enum.TextXAlignment.Left
infoLabel.Parent = fakeContainer
...
local function createUtilButton(text, callback, order)
    local btn = Instance.new("TextButton")
...
local function startAutoRejoin()
    if not autoRejoinEnabled or isRejoining then return end
    ...
    task.spawn(function()
        ...
        while isRejoining and IsRunning do
...
local matchRemote = remotes:WaitForChild("MatchUI")
...
matchRemote.OnClientEvent:Connect(function(event)
    ...
    if event == "StartTurn" or event == "YourTurn" then
        ...
        if not isTyping then
            task.wait(turnDelay + math.random() * (rejoinDelay - turnDelay))
            autoType()
        end
...
local function applyFakeName(name)
    ...
    if character and character:FindFirstChild("Humanoid") then
        ...
        for _, obj in pairs(character:GetDescendants()) do
            if obj:IsA("TextLabel") and (obj.Text == LocalPlayer.Name or obj.Text == LocalPlayer.DisplayName or obj.Text == currentFakeName) then
                obj.Text = name
            end
        end
        
        currentFakeName = name
        print("🎭 Fake Name '" .. name .. "' diterapkan!")
    end
end

applyBtn.MouseButton1Click:Connect(function()
    applyFakeName(nameBox.Text)
    playClickSound()
end)
...
local themeList = {"CYBERPUNK", "CRIMSON", "MATRIX", "SAKURA", "OCEAN", "FLAME"}
...
local function updateThemeDropdown()
    ...
    for i, themeName in ipairs(themeList) do
        ...
        nameLabel.Text = themeName .. (CurrentTheme == themeName and "  ✓" or "")
...
themeBtn.MouseButton1Click:Connect(function()
    ...
    themeBtn.Text = "🎨  TEMA: " .. CurrentTheme .. (themeOpen and " ▲" or " ▼")
...
createUtilButton("⭐  REJOIN SERVER", function()
...
skinBtn.Size = UDim2.new(1, 0, 0, IsMobile and 42 or 38)
skinBtn.LayoutOrder = 6
skinBtn.BackgroundColor3 = Color3.fromRGB(25, 22, 38)
skinBtn.Text = "🎋  SKIN BAMBU: (pilih) ▼"
skinBtn.TextColor3 = Color3.new(1, 1, 1)
skinBtn.Font = Enum.Font.GothamBold
skinBtn.TextSize = TEXT_SIZE_NORMAL
skinBtn.Parent = utilContainer

local skinBtnCorner = Instance.new("UICorner")
skinBtnCorner.CornerRadius = UDim.new(0, 9)
skinBtnCorner.Parent = skinBtn

local skinBtnGradient = Instance.new("UIGradient")
skinBtnGradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(90, 30, 190)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(50, 15, 120))
})
skinBtnGradient.Parent = skinBtn

local skinBtnStroke = Instance.new("UIStroke")
skinBtnStroke.Color = Color3.fromRGB(150, 80, 255)
skinBtnStroke.Thickness = 1.5
skinBtnStroke.Transparency = 0.2
skinBtnStroke.Parent = skinBtn

UI_Elements.SkinDropBtn = skinBtn
UI_Elements.SkinDropGrad = skinBtnGradient
UI_Elements.SkinDropStroke = skinBtnStroke

local skinDropdown = Instance.new("Frame")
skinDropdown.Size = UDim2.new(1, 0, 0, 0)
skinDropdown.BackgroundColor3 = Color3.fromRGB(14, 13, 22)
skinDropdown.ClipsDescendants = true
skinDropdown.BorderSizePixel = 0
skinDropdown.LayoutOrder = 7
skinDropdown.Parent = utilContainer

local skinDropdownCorner = Instance.new("UICorner")
skinDropdownCorner.CornerRadius = UDim.new(0, 9)
skinDropdownCorner.Parent = skinDropdown

local skinDropdownStroke = Instance.new("UIStroke")
skinDropdownStroke.Color = Color3.fromRGB(70, 35, 130)
skinDropdownStroke.Thickness = 1
skinDropdownStroke.Transparency = 0.4
skinDropdownStroke.Parent = skinDropdown

UI_Elements.SkinFrameStroke = skinDropdownStroke

local skinInfo = Instance.new("TextLabel")
skinInfo.Size = UDim2.new(1, -12, 0, 20)
skinInfo.Position = UDim2.new(0, 8, 1, 6)
...
skinInfo.TextSize = 10
skinInfo.TextXAlignment = Enum.TextXAlignment.Left
skinInfo.Parent = utilContainer
skinInfo.LayoutOrder = 8

UI_Elements.SkinInfoText = skinInfo
...
local function applySkin(skinName)
    ...
    if not backWeapon then return end
...
local function updateSkinDropdown()
    ...
    for i, skinData in ipairs(SKINS) do
        local item = Instance.new("Frame")
        item.Size = UDim2.new(1, -12, 0, IsMobile and 34 or 30)
        item.Position = UDim2.new(0, 6, 0, (i - 1) * skinItemHeight + 6)
        item.BackgroundColor3 = currentSkin == skinData.name and Color3.fromRGB(30, 25, 50) or Color3.fromRGB(20, 18, 30)
        item.BorderSizePixel = 0
        ...
        nameLabel.Position = UDim2.new(0, 28, 0, 0)
        nameLabel.BackgroundTransparency = 1
        nameLabel.Text = skinData.label .. (currentSkin == skinData.name and "  ✓" or "")
        nameLabel.TextColor3 = currentSkin == skinData.name and skinData.color or Color3.fromRGB(200, 190, 220)
        nameLabel.Font = Enum.Font.GothamBold
        nameLabel.TextSize = TEXT_SIZE_NORMAL
        nameLabel.TextXAlignment = Enum.TextXAlignment.Left
        nameLabel.Parent = item
        ...
        rarityLabel.Size = UDim2.new(0, 105, 1, 0)
        ...
        selectBtn.MouseButton1Click:Connect(function()
            ...
            skinInfo.TextColor3 = RARITY_COLORS[skinData.rarity] or THEMES[CurrentTheme] and THEMES[CurrentTheme].logText or Color3.new(1, 1, 1)
...
skinBtn.MouseButton1Click:Connect(function()
    ...
    skinDropdown:TweenSize(
        UDim2.new(1, 0, 0, skinOpen and #SKINS * skinItemHeight + 12 or 0),
        Enum.EasingDirection.Out,
        Enum.EasingStyle.Quart,
        0.25,
        true
...
fakeTab.MouseButton1Click:Connect(function()
    switchTab(fakeContainer)
    highlightTab(fakeTab)
    playClickSound()
end)
